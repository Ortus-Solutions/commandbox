...
component accessors="true"{
	...
	*/
	function configure( required injector, required properties ){
		...
		// mixer util
		variables.mixerUtil 			= new wirebox.system.aop.MixerUtil();
		// class id code
		...
		if( NOT structKeyExists( variables.properties, "generationPath" ) ){
			variables.properties.generationPath = "/wirebox/system/aop/tmp";
		}
	...
	private function weaveAdvice(
		...
		>
			<cfscript>
				...
				var invocation = createObject( "component", "wirebox.system.aop.MethodInvocation" ).init(
