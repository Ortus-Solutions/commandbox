/**
 * View properties set in box.json for this package using JMESPath.  Command must be executed from the root
...
component {
	...
	function run( string property='', boolean system=false ) {
		...
		try {
			...
			var propertyValue = JSONService.showJMES( boxJSON, arguments.property );
