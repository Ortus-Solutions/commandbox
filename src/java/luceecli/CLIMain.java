...
public class CLIMain{
	...
	public static void main( String[] args ) throws ServletException,
		...
		File currentDir = new File( CLIMain.class.getProtectionDomain()
		...
		if( !Util.isEmpty( strDebug, true ) ) {
			debug = true;
		}

		if( debug ) {
			System.out.println( "libDir dir" + libDir.getPath() );
		}
		...
		// fix for windows. remove drive letter. TODO: find out why this is
		// needed
		config.put( "uri", new File( config.get( "uri" ) ).toURI().toURL()
				.toExternalForm().replaceAll( "file:/(\\w:)", "file://$1" ) );
		...
		initParameters.put( "lucee-server-directory",
				serverRoot.getAbsolutePath() );
		initParameters.put( "configuration", webConfig.getAbsolutePath() );

		CLIContext servletContext = new CLIContext( root, webConfig,
				attributes, initParameters, 1, 0 );
	...
	// java lucee-cli.jar -config=.../lucee-web.xml.cfm
	// -uri=/susi/index.cfm?test=1 -form=name=susi -cgi=user_agent=urs
	// -output=.../test.txt ...
	/**
	 * Config
	 * 
	 * webroot - webroot directory servlet-name - name of the servlet
	 * (default:CFMLServlet) server-name - server name (default:localhost) uri -
	 * host/scriptname/query cookie - cookies (same pattern as query string)
	 */
	/**
	 * @param uri
	 * @param debug
	 ...
	 * @throws JspException
	 */
	public static void run( File webroot, File configServerDir,
			File configWebDir, String uri, boolean debug )
		...
		config.put( "uri", new File( uri ).toURI().toURL().toExternalForm()
				.replaceAll( "file:/(\\w:)", "file://$1" ) );
		String servletName = config.get( "servlet-name" );
		if( Util.isEmpty( servletName, true ) ) {
			servletName = "CFMLServlet";
		}
		...
		CLIContext servletContext = new CLIContext( webroot, configWebDir,
				attributes, initParameters, 1, 0 );
		ServletConfigImpl servletConfig = new ServletConfigImpl(
				servletContext, servletName );
		PrintStream printStream = new PrintStream( new ByteArrayOutputStream() );
