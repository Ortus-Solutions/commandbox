...
Windows executables are signed automatically during `buildInstallers` when `SIGN_WINDOWS_EXE=true` (the default) and signing credentials are configured. Set these environment variables before building:

```powershell
$env:SIGN_WINDOWS_EXE="true"
$env:DIGICERT_API_KEY="YOUR_API_KEY"
$env:DIGICERT_CLIENT_CERTIFICATE_BASE64="BASE64_ENCODED_CLIENT_CERTIFICATE"
$env:DIGICERT_CLIENT_CERTIFICATE_PASSWORD="YOUR_CERTIFICATE_PASSWORD"
$env:DIGICERT_KEY_ALIAS="YOUR_KEY_ALIAS"
```

Signing uses Sign4j -> Jsign -> DigiCert KeyLocker. The client certificate is decoded from `DIGICERT_CLIENT_CERTIFICATE_BASE64` into a temporary file and removed after signing. If `DIGICERT_API_KEY` is not configured, signing is skipped even when `SIGN_WINDOWS_EXE=true`.
