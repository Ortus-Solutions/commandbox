...
actual=$(docker run --rm \
	...
	ubuntu:24.04 sh -c 'apt-get update >/dev/null && apt-get install --yes --no-install-recommends openjdk-21-jre-headless bash curl unzip >/dev/null && cp /mnt/box /usr/local/bin/box && chmod 755 /usr/local/bin/box && /usr/local/bin/box version' | tail -n 1)
