component {
	...
	function compileCommand() {
		...
		var jarFile = resolvePath( "libs/command-compile.jar" );
	...
	function nonStandardSourcePath() {
		...
		assertFileExists( "classes/java/main/example/custom/SourcePathApp.class" );
		assertFileExists( "libs/non-standard-source.jar" );
		classLoad( resolvePath( "libs/non-standard-source.jar" ) );
	...
	function multipleSourcePaths() {
		...
		assertJarEntryExists( resolvePath( "libs/multiple-sources.jar" ), "example/App.class" );
		assertJarEntryExists( resolvePath( "libs/multiple-sources.jar" ), "example/custom/SourcePathApp.class" );
	}
	...
	function sourceGlob() {
		...
		assertJarEntryExists( resolvePath( "libs/source-glob.jar" ), "example/custom/SourcePathApp.class" );
	}
	...
	function defaultResources() {
		...
		assertJarEntryExists( resolvePath( "libs/default-resources.jar" ), "default-resource.txt" );
		assertJarEntryExists( resolvePath( "libs/default-resources.jar" ), "nested/default-nested-resource.txt" );
	}
	...
	function packageMetadata() {
		...
		assertFileExists( "package-fixture/libs/compile-dsl-package-1.2.3.jar" );
		assertJarManifestValue( resolvePath( "package-fixture/libs/compile-dsl-package-1.2.3.jar" ), "Bundle-Name", "Compile DSL Package" );
		assertJarManifestValue( resolvePath( "package-fixture/libs/compile-dsl-package-1.2.3.jar" ), "Bundle-SymbolicName", "compile-dsl-package" );
		assertJarManifestValue( resolvePath( "package-fixture/libs/compile-dsl-package-1.2.3.jar" ), "Bundle-Version", "1.2.3" );
		assertJarManifestValue( resolvePath( "package-fixture/libs/compile-dsl-package-1.2.3.jar" ), "Implementation-Title", "Package Manifest Override" );
	}
	...
	function customClassOutput() {
		...
		assertJarEntryExists( resolvePath( "libs/custom-class-output.jar" ), "example/App.class" );
	}
	...
	function resources() {
		...
		assertJarEntryExists( resolvePath( "libs/resources.jar" ), "message.txt" );
	}
	...
	function customManifest() {
		...
		assertJarManifestValue( resolvePath( "libs/custom-manifest.jar" ), "Implementation-Title", "Compile DSL Test" );
	}
	...
	function jarOptionValues() {
		...
		assertJarEntryCompression( resolvePath( "libs/jar-option-values.jar" ), "example/App.class", 0 );
		assertJarManifestValue( resolvePath( "libs/jar-option-values.jar" ), "Main-Class", "example.App" );
	}
	...
	function classPath() {
		...
		assertJarEntryExists( resolvePath( "libs/classpath.jar" ), "example/classpath/ClasspathApp.class" );
		classLoad( [ result.classPaths.first(), resolvePath( "libs/classpath.jar" ) ] );
	...
	function multipleClassPaths() {
		...
		// java compile source=src/multiple-classpath/java classPath=libs/compile-dsl.jar,libs/maven classes=classes/multiple-classpaths jarName=multiple-classpaths.jar --jar --JSON
		runCompile( { source="src/multiple-classpath/java", classPath="libs/compile-dsl.jar,libs/maven", classes="classes/multiple-classpaths", jarName="multiple-classpaths.jar" }, [ "jar", "JSON" ] );
		classLoad( resolvePath( "libs/multiple-classpaths.jar" ) );
	...
	function classPathList() {
		// java compile source=src/multiple-classpath/java classPath=libs,libs/maven classes=classes/classpath-list jarName=classpath-list.jar --jar --JSON
		runCompile( { source="src/multiple-classpath/java", classPath="libs,libs/maven", classes="classes/classpath-list", jarName="classpath-list.jar" }, [ "jar", "JSON" ] );
	...
	function javaDocs() {
		...
		assertFileExists( "javaDocs/main/index.html" );
		assertEquals( resolvePath( "javaDocs/main" ), result.javaDocsPath );
	}

	/** @test */
	function fatJar() {
		var localDependencyJar = "libs/compile-dsl-fat-#createUUID()#.jar";
		runCompile( { jarName=listLast( localDependencyJar, "/\\" ) }, [ "jar", "JSON" ] );
		prepareMavenDependency();
		var dependencyJar = directoryList( "libs/maven", true, "array", "*.jar" ).first();
		// java compile source=src/multiple-classpath/java classPath=libs/compile-dsl-fat-<uuid>.jar,libs/maven classes=classes/fat-jar fatJarName=fat-jar.jar fatJarJars=libs/compile-dsl-fat-<uuid>.jar,#dependencyJar# --fatJar --JSON
		...
		assertJarEntryExists( resolvePath( "libs/fat-jar.jar" ), "example/multipleclasspath/MultiClasspathApp.class" );
		assertJarEntryExists( resolvePath( "libs/fat-jar.jar" ), "example/App.class" );
		assertJarEntryExists( resolvePath( "libs/fat-jar.jar" ), "org/apache/commons/lang3/StringUtils.class" );
	}
	...
	function thinAndFatJar() {
		...
		prepareMavenDependency();
		var dependencyJar = directoryList( "libs/maven", true, "array", "*.jar" ).first();
		// java compile source=src/multiple-classpath/java classPath=libs/compile-dsl.jar,libs/maven jarName=thin-and-fat.jar fatJarName=thin-and-fat-all.jar fatJarJars=libs/compile-dsl.jar,#dependencyJar# --JSON
		var result = runCompile( { source="src/multiple-classpath/java", classPath="libs/compile-dsl.jar,libs/maven", jarName="thin-and-fat.jar", fatJarName="thin-and-fat-all.jar", fatJarJars="libs/compile-dsl.jar,#dependencyJar#" }, [ "JSON" ] );
		assertFileExists( "libs/thin-and-fat.jar" );
		assertFileExists( "libs/thin-and-fat-all.jar" );
		assertJarEntryMissing( resolvePath( "libs/thin-and-fat.jar" ), "org/apache/commons/lang3/StringUtils.class" );
		assertJarEntryExists( resolvePath( "libs/thin-and-fat-all.jar" ), "org/apache/commons/lang3/StringUtils.class" );
	}

	/** @test */
	function fatJarServiceDescriptors() {
		if( fileExists( "libs/fat-jar-services.jar" ) ) fileDelete( "libs/fat-jar-services.jar" );
		...
		assertJarEntryExists( resolvePath( "libs/fat-jar-services.jar" ), "META-INF/services/example.Service" );
		assertJarEntryMissing( resolvePath( "libs/fat-jar-services.jar" ), "META-INF/DEPENDENCY.SF" );
		assertEquals( "example.provider.First#chr(10)#example.provider.Second", readJarEntry( resolvePath( "libs/fat-jar-services.jar" ), "META-INF/services/example.Service" ) );
	}
	...
	function fatJarDuplicateResources() {
		...
		assertEquals( "second dependency", readJarEntry( resolvePath( "libs/fat-jar-duplicates.jar" ), "duplicate-resource.txt" ) );
	}
	...
	function fatJarMergeOptions() {
		...
		assertJarEntryExists( resolvePath( "libs/fat-jar-options.jar" ), "META-INF/DEPENDENCY.SF" );
		assertEquals( "example.provider.Second", readJarEntry( resolvePath( "libs/fat-jar-options.jar" ), "META-INF/services/example.Service" ) );
	}
	...
	function fatJarDuplicatePolicies() {
		...
		assertEquals( "first dependency", readJarEntry( resolvePath( "libs/fat-jar-policy-first-wins.jar" ), "duplicate-resource.txt" ) );
	...
	function jarOptions() {
		...
		assertJarEntryCompression( resolvePath( "libs/jar-options.jar" ), "example/App.class", 0 );
	}
	...
	function sourceEncoding() {
		...
		classLoad( resolvePath( "libs/source-encoding.jar" ) );
